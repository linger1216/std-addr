...
export const complainsRouter = createTRPCRouter({
  ...
  /**
   * 人房关联(仅原始行):返回非匿名诉件原始行(不做 ML 标准化 / 不建树)。
   * 供前端分块增量分析:前端拿到行后,按 chunk 调 mlFieldsBatch 标准化,
   * 每批回来即重建人房树并更新 UI,实现"分析过程中树逐步生长"。
   */
  personHouseRows: protectedProcedure
    .input(
      z.object({
        startDate: z.string().trim().optional(),
        endDate: z.string().trim().optional(),
  ...
  /**
   * 批量 ML 要素解析:对一批地址(优先 std_address,回退 address)做 NER,
   * 返回与入参顺序一致的 AddrFields[]。单条失败回退 {},不影响整体。
   * 前端按 chunk 调用,实现人房树"边算边显"。
   */
  mlFieldsBatch: protectedProcedure
    .input(z.object({ addresses: z.array(z.string()).max(500) }))
    .query(async ({ input }) => {
      const results = await Promise.all(
        input.addresses.map(async (addr): Promise<AddrFields> => {
          if (!addr?.trim()) return {};
          try {
            const std = await standardizeService.mlFields(addr);
            return {
              community: std.community,
              poi: std.poi,
              village: std.village,
