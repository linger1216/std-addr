...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-10 px-4 text-left align-middle text-[12px] font-medium uppercase tracking-wide text-muted-foreground [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "px-4 py-3 align-middle text-[13px] text-foreground [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
