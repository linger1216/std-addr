/**
 * 道路页 UI 状态 store —— 模块私有(对齐 community-store 模板)。
 */
...
export type RoadSortId = "road" | "status" | "createdAt";
...
export const useRoadStore = create<State & Actions>()((set, get) => ({
  page: 1,
  pageSize: 20,
  sorting: [],
  rowSelection: {},
...
export function useRoadState(): UiSlice {
  return useRoadStore(
    useShallow((s) => ({
      page: s.page,
      pageSize: s.pageSize,
...
export function useRoadActions(): ActionsSlice {
  return useRoadStore(
    useShallow((s) => ({
      setPage: s.setPage,
