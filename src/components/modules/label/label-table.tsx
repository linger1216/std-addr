...
export type LabelRow = RouterOutputs["label"]["list"]["items"][number];

const columnHelper = createAppColumnHelper<LabelRow>();

/** 行级回调 —— 通过 column meta 透传给列定义使用 */
type LabelRowCallbacks = {
  onView?: (row: LabelRow) => void;
 ...
 */
export function createLabelColumns() {
  return columnHelper.columns([
    ...
    columnHelper.display({
      ...
      cell: ({ row, table }) => {
        ...
        const cb = (table.options.meta as { callbacks?: LabelRowCallbacks })
          ?.callbacks;
        return (
 ...
 */
export const LabelTable = memo(function LabelTable({
  table,
  isLoading,
  callbacks,
  columnSizing,
  ...
  const metaRef = useRef<{ callbacks: LabelRowCallbacks }>({ callbacks });
