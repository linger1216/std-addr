/**
 * POI 页 UI 状态 store —— 模块私有(对齐 community-store 模板)。
 */
...
export const usePoiStore = create<State & Actions>()((set, get) => ({
  page: 1,
  pageSize: 20,
  sorting: [],
  rowSelection: {},
...
export function usePoiState(): UiSlice {
  return usePoiStore(
    useShallow((s) => ({
      page: s.page,
      pageSize: s.pageSize,
...
export function usePoiActions(): ActionsSlice {
  return usePoiStore(
    useShallow((s) => ({
      setPage: s.setPage,
