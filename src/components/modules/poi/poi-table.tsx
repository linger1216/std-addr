...
export type PoiRow = RouterOutputs["poi"]["list"]["items"][number];

const columnHelper = createAppColumnHelper<PoiRow>();

/** 行级回调 —— 通过 column meta 透传给列定义使用 */
type PoiRowCallbacks = {
 ...
 */
export function createPoiColumns() {
  return columnHelper.columns([
    ...
    columnHelper.display({
      ...
      cell: ({ row, table }) => {
        ...
        const cb = (table.options.meta as { callbacks?: PoiRowCallbacks })
          ?.callbacks;
        return (
...
/** 地址列:多行渲染(对齐 community.address 列) */
 ...
 */
export const PoiTable = memo(function PoiTable({
  table,
  isLoading,
  callbacks,
}: {
  ...
  const metaRef = useRef<{ callbacks: PoiRowCallbacks }>({ callbacks });
