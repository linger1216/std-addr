/**
 * 子区域页 UI 状态 store —— 模块私有。
 *
 * 拆分:
 *   - 查询参数(搜索/区划/状态):useSubareaQueryParams(createQueryParamsStore,与 road/poi/village 一致)
 *   - UI 态(分页/排序/选中/dialog open):本 store,纯展示状态
 *   - 排序 id 白名单:见 SubareaSortId
 *
 * 用法:
 *   const { page, setPage, rowSelection, openForm, closeForm } = useSubareaState();
...
export const useSubareaStore = create<State & Actions>()((set, get) => ({
  page: 1,
  pageSize: 20,
  sorting: [],
  rowSelection: {},
...
export function useSubareaState(): UiSlice {
  return useSubareaStore(
    useShallow((s) => ({
      page: s.page,
      pageSize: s.pageSize,
      sorting: s.sorting,
...
export function useSubareaActions(): ActionsSlice {
  return useSubareaStore(
    useShallow((s) => ({
      setPage: s.setPage,
