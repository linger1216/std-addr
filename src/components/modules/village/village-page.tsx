...
import { VillageToolbar, type RegionOption } from "./village-toolbar";
import { VillageDetailDialog } from "./village-detail";
...
import { useVillageState, useVillageActions } from "./stores/village-store";
...
/**
 * 村页顶层编排(以 community 为模板)。
 *
 * 数据流:
 *   useVillageQueryParams (store) → submitted query params
 *     ↓
 *   tRPC useQuery (list / stats / regions / getById)
 *     ↓
 *   useCrudTable (table 实例 + 选中 + 分页/排序)
 *     ↓
 *   VillageTable / Toolbar / PaginationControl / Stats / Dialogs
 *
 * 副作用流:
...
export function VillagePage() {
  ...
  const filters = useVillageQueryParams((s) => s.committed);

  // —— 2. UI store(分页/排序/选中/dialog)——
  const state = useVillageState();
  const actions = useVillageActions();
  ...
  const mut = useCrudMutations({
    utils,
    invalidateKeys: ["village"],
    procedures: {
      create: api.village.create,
      update: api.village.update,
      delete: api.village.delete,
      deleteMany: api.village.deleteMany,
    },
    messages: {
      createSuccess: "村已创建",
      updateSuccess: "村已更新",
  ...
  const { data: listData, isLoading: listLoading } =
    api.village.list.useQuery(listQueryParas);
  ...
  const { table, selectedIds, columnSizing, onColumnSizingChange } = useCrudTable<VillageRow>({
    data: rows,
    columns,
    getRowId: (r) => r.id,
    pageSize: state.pageSize,
  ...
  const excel = useCrudExcel<VillageRow, ImportRowInput, ImportInput>({
    moduleName: "村",
    exportColumns: [
      { header: "名称", width: 24 },
      { header: "别名", width: 20 },
    ...
    exportRow: (r) => ({
      "名称": r.name,
      // alias 是 JSON 数组:导出时拼成 " / " 分隔的可读文本(对齐 community.address)
      "别名": parseAliasEntries(r.alias).join(" / "),
      "所属区划ID": r.regionId ?? "",
      "状态": r.status === 1 ? 1 : 0,
    }),
    fetchAll: () =>
      utils.village.exportAll
        .fetch({
          q: filters.q ?? undefined,
          regionId: filters.regionId ?? undefined,
          status:
        ...
        .then((items) => items as VillageRow[]),
    importFields: [
    ...
    wrapInput: (rows) => ({ rows }),
    importMutation: api.village.import,
  });
  ...
  // —— 11. 派生显示数据 ——
  const editingFormInitial: VillageDetail | null =
    state.editingId && editingData ? editingData : null;
  const detail: VillageDetail | null = detailData ?? null;
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {`确定删除村「${state.deleteRow?.name ?? ""}」?此操作不可恢复。`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {`确定删除选中的 ${selectedIds.length} 个村?此操作不可恢复。`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
