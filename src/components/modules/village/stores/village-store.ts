 ...
 *
 * 拆分:
 *   - 查询参数(搜索/区划/状态):useVillageQueryParams(createQueryParamsStore)
 *   - UI 态(分页/排序/选中/dialog open):本 store,纯展示状态
 *   - 排序 id 白名单:见 VillageSortId
 *
 * 用法:
...
export const useVillageStore = create<State & Actions>()((set, get) => ({
  page: 1,
  pageSize: 20,
  sorting: [],
  rowSelection: {},
...
export function useVillageState(): UiSlice {
  return useVillageStore(
    useShallow((s) => ({
      page: s.page,
      pageSize: s.pageSize,
      sorting: s.sorting,
...
export function useVillageActions(): ActionsSlice {
  return useVillageStore(
    useShallow((s) => ({
      setPage: s.setPage,
